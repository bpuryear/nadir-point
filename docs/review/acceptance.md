...
| Three loadouts of the same cruiser distinguishable in silhouette | **FAIL** | Recorded as PASS against figures that were **below the probe's own stated targets even as quoted**, while the probe itself printed FAIL. Corrected. Current, from `docs/probes/loadouts.png` and `-top.png`: A/B mean 40.0 max 381, A/C mean 32.6 max 274, B/C mean 26.5 max 381. Worst pair mean **26.5 against a target of 45** — FAIL. Max now passes (274 ≥ 120), so there *are* places on the outline where two builds differ decisively; what is missing is difference along most of the length. The three builds are still the same body wearing different hats. Closing this needs larger, more cantilevered ventral and engine modules in `src/art/geometry/modules/**` — the hull's own bands are already as differentiated as one hull can be. |
